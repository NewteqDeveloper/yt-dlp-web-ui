...
export const appTitleState = atomWithStorage(
  'appTitle',
  localStorage.getItem('appTitle') ?? `Sanity's YouTube Downloader`
);

export const serverAddressAndPortState = atom((get) => {
  if (get(servedFromReverseProxySubDirState)) {
    return `${get(serverAddressState)}/${get(
...
export const serverURL = atom(
  (get) => `${window.location.protocol}//${get(serverAddressAndPortState)}`
);
...
export const serverSideCookiesState = atom<Promise<string>>(
  async (get) =>
    await pipe(
